# -*- coding: utf-8 -*-
# @Time    : 2024/7/18 20:25
# @Author  : Ginger
# @FileName: DMC_LICAG_test.py
# @Software: PyCharm
'''
...
def Training():
    ...
    for epoch in tqdm.tqdm(range(1000)):
        ...
        for view_index in range(args.viewNumber):
            mse_loss += torch.nn.functional.mse_loss(output[view_index][0], x[view_index])
        ...
        for view_index in range(args.viewNumber):
            ...
            # input = torch.log_softmax(qlist[view_index], dim=0)  # 对数概率分布,dim=0在行上softmax
            # target = torch.softmax(plist[view_index],dim=0)  # 概率分布
        ...
        Loss = 1 * mse_loss + args.gamma * kl_loss
        optimizer.zero_grad()
        Loss.backward()
        optimizer.step()
